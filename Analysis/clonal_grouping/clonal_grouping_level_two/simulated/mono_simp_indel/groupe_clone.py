...
def group_same_VJ(lines,formatted_file):
	...
	for l in range(0,len(lines)):
		...
		Clone_identity = str(NumClone[1].split("*")[0]) # same V gene
		#print (Clone_identity)
		...
		for seq in dico_same_VJ[VJ_ID]:
			...
			if len(VJ_ID_diff_CDR3[VJ_ID].keys()) != 0 :
				...
				for g in Sub_gourp :
					...
					print("aaaaa",1 -hamming_distance(dicoSeq[seq.rstrip()][2] , g)/float(len(g)))
					if len(dicoSeq[seq.rstrip()][2]) == len(g) :
						if 1-(hamming_distance(dicoSeq[seq.rstrip()][2] , g)/float(len(g))) >= Clone_threshold :
							print("here!")
							sub_gourp_dist[g] =('+',1-(hamming_distance(dicoSeq[seq.rstrip()][2] , g)/float(len(g))))

					elif dicoSeq[seq.rstrip()][1].split("*")[0][-1] == "6" :
						length = max(len(dicoSeq[seq.rstrip()][2]),len(g))
						if 1-(levenshtein_distance(dicoSeq[seq.rstrip()][2] , g)/float(length)) >= Clone_threshold :
							sub_gourp_dist[g] =('+',1-(levenshtein_distance(dicoSeq[seq.rstrip()][2] , g)/float(length)))
...
def main():
    usage = "python  groupe_clone.py -i <formated IMGT highvquest statistics output> -o <output file name> -s <Clone identity percentage> \n "
