...
def CalculateMedoid(dico_vjunc,Dicoresult):
	...
	for key in Dicoresult.keys() :
		listloc=[]
		print("Dicoresult[key]",Dicoresult[key])
		for seq in Dicoresult[key]:
			if seq.rstrip() in dico_vjunc.keys():
				print("dico_vjunc[seq.rstrip()][2]",dico_vjunc[seq.rstrip()][2])
...
def Creat_dico_neighbour(Dicocentroid):
	...
	print("dicoNeighbour",dicoNeighbour)
...
def write_clone_V_cdr3_(Dicoresult,dicoSeq,uniq_seq_dico,output_file):
	...
	for cluster in Dicoresult.keys():
		#print VJ
		for seq in Dicoresult[cluster]:
				print("skpf,gksnpkg",dicoSeq[seq][2].split(" ")[0])
				sequence = str(clone_number) + "\t" + str(seq) + "\t" +dicoSeq[seq][0] + "\t" + dicoSeq[seq][1] + "\t" +dicoSeq[seq][2].split(" ")[0] +"\t" +dicoSeq[seq][3]+ "\n"
				filetowrite.write(sequence)
				if len(uniq_seq_dico[seq]) != 0 :
					for dup in uniq_seq_dico[seq] :
						sequence = str(clone_number) + "\t" + str(dup) + "\t" +dicoSeq[seq][0] + "\t" + dicoSeq[seq][1] + "\t" +dicoSeq[seq][2].split(" ")[0] +"\t" +dicoSeq[seq][3]+ "\n"
						filetowrite.write(sequence)
		clone_number += 1
...
def main():
	...
	#dico_vjunc, len_max_CDR3,len_max_J =read_vjunction(read_file (ClusteringFile))

	#print (dico_vjunc['S10692'])
	...
	#filtered_clustering_label = dico_result
	...
	#sil = silhouette(dico_vjunc,dico_centroid,filtered_clustering_label,dico_neighbour,len_max_CDR3,len_max_J,dico_vjunc)
